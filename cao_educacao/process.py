...
def find_school_type(content):
    pat_estadual = re.compile(
        r'(col[ée]gio|escola)\s+estadual',
        re.IGNORECASE
    )
    pat_municipal = re.compile(
        r'(col[ée]gio|escola|plano|centro|creche)\s+(municipal|municipalizada)',
        re.IGNORECASE
    )

    if pat_estadual.search(content):
        return 'estadual'
    elif pat_municipal.search(content):
        return 'municipal'
    else:
        return 'privada'
...
def main(doc_info):
    ...
    school_type = find_school_type(get_first_page(doc_info.content))
    craai = craais[city_name.upper()]
    ...
    if 'classe_nao_encontrada' in complete_output_path:
...
if __name__ == '__main__':
    ...
    OUTPUT_PATH = os.path.join(
        out_path,
        '{year}/{craai}/{city}/{school_type}/' + doc_type + '/{keyword}'
    )
